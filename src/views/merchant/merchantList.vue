...
export default {
  ...
  methods: {
    ...
    examine(row, index, done, loading) {
      //做审核
      row.mtid = row.mtid; //重新给mtid赋值
      update(
        row.miid,
    ...
    onLoad(page, params = {}) {
      ...
      getList(
        ...
        if (res && res.data) {
          ...
          if (data.merchantInfoList) {
            data.merchantInfoList.sort(
              (a, b) => Number(a.status) - Number(b.status)
            ); //按启用禁用排序
            this.data = data.merchantInfoList;
          }
